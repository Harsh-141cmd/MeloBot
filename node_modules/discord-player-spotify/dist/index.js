...
var SpotifyAPI = class _SpotifyAPI {
  ...
  async requestToken() {
    try {
      ...
      try {
        ...
        const expiresAfter = Number(body.match(/"accessTokenExpirationTimestampMs":(\d+)/)?.[1]) || 1e3 * 60 * 60;
   ...
   */
  static async getTracksFromEmbed(url, type = "track") {
    const embedUrl = _SpotifyAPI.getEmbedLink(url, type);
    ...
    switch (type) {
      ...
      case "playlist": {
        ...
        playlist.tracks = jsonData.props.pageProps.state.data.entity.trackList.map((item) => {
          const t = {};
          t.title = item.title;
...
SpotifyExtractor.identifier = "com.discord-player.itsmaat.spotifyextractor";
export {
  SpotifyExtractor,
  parseSpotifyUrl
};
